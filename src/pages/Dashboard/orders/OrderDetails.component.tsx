...
import { TOrdersProduct, TProductGetApiResponse } from '@/types';
...
export function OrderDetailsDialog({ viewDetails, setViewDetailsClose }: OrderDetailsDialogProps) {
  ...
  const columns = [
    ...
    { label: 'Quantity', key: 'quantity' },
    {
      label: 'Subtotal',
      key: 'subtotal',
      render: (_: unknown, item: TOrdersProduct) => `$${((item.productId.price * item.quantity)).toFixed(2)}`
    }
  ];
              ...
              <Card>
                ...
                <CardContent>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span>{order.products.reduce((sum: number, item: TProductGetApiResponse) => sum + item.quantity, 0)}</span>
                  </div>
